...
func main() {
	...
	if cfg.GroupsEnabled {
		if cfg.GroupsSecretKey == "" {
			...
			if !cfg.GroupsAllowPublicCreation && len(cfg.GroupsAdminPubkeys) == 0 {
				log.Printf("Warning: NIP-29 groups enabled but GROUPS_ALLOW_PUBLIC_CREATION is false and GROUPS_ADMIN_PUBKEYS is empty - no one can create groups")
			}
			...
			groupsState.AllowAction = groups.TrackingAllowAction
			...
			r.RejectEvent = append(r.RejectEvent,
				...
				// PermitRolelessPutUser wraps RestrictInvalidModerationActions to allow
				// roleless members to invite (PutUser). See ownership.go for details.
				groups.PermitRolelessPutUser(groupsState),
